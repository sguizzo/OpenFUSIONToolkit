...
!> @file mugtok_f.F90
!
!> Fortran part of Python wrapper for coupled MUG/TokaMaker (blanket_td) functionality
!!
...
USE oft_blanket_td, ONLY: oft_mugtok_td
...
!---------------------------------------------------------------------------------
!> Get the number of DOFs (node points) for a given solution field
!---------------------------------------------------------------------------------
SUBROUTINE mugtok_field_ndofs(mugtok_ptr,field_id,n,error_str) BIND(C,NAME="mugtok_field_ndofs")
TYPE(c_ptr), VALUE, INTENT(in) :: mugtok_ptr !< Pointer to MUG/TokaMaker object
INTEGER(c_int), VALUE, INTENT(in) :: field_id !< Field index (1=n,2/3/4=vel,5=P,7=F)
INTEGER(c_int), INTENT(out) :: n !< Number of DOFs for the field
CHARACTER(KIND=c_char), INTENT(out) :: error_str(OFT_ERROR_SLEN) !< Error string (empty if no error)
TYPE(mugtok_instance), POINTER :: sim_obj
REAL(r8), POINTER, DIMENSION(:) :: buf
n=0
IF(.NOT.mugtok_ccast(mugtok_ptr,sim_obj,error_str))RETURN
NULLIFY(buf)
CALL sim_obj%sim%u%get_local(buf,field_id) ! get_local allocates buf to the block size
n=SIZE(buf)
DEALLOCATE(buf)
END SUBROUTINE mugtok_field_ndofs
!---------------------------------------------------------------------------------
...
TYPE(c_ptr), VALUE, INTENT(in) :: mugtok_ptr !< Pointer to MUG/TokaMaker object
INTEGER(c_int), VALUE, INTENT(in) :: field_id !< Field index (1=n,2/3/4=vel,5=P,7=F)
TYPE(c_ptr), VALUE, INTENT(in) :: vals !< Output array (size = mugtok_field_ndofs)
CHARACTER(KIND=c_char), INTENT(out) :: error_str(OFT_ERROR_SLEN) !< Error string (empty if no error)
TYPE(mugtok_instance), POINTER :: sim_obj
REAL(r8), POINTER, DIMENSION(:) :: buf,vals_tmp
IF(.NOT.mugtok_ccast(mugtok_ptr,sim_obj,error_str))RETURN
NULLIFY(buf)
CALL sim_obj%sim%u%get_local(buf,field_id)
CALL c_f_pointer(vals,vals_tmp,[SIZE(buf)])
vals_tmp=buf
DEALLOCATE(buf)
END SUBROUTINE mugtok_get_field
!---------------------------------------------------------------------------------
!> Get the L2-projected poloidal gradient of F at node points (dF/dR, dF/dZ)
!---------------------------------------------------------------------------------
SUBROUTINE mugtok_get_gradf(mugtok_ptr,gr,gz,error_str) BIND(C,NAME="mugtok_get_gradf")
TYPE(c_ptr), VALUE, INTENT(in) :: mugtok_ptr !< Pointer to MUG/TokaMaker object
TYPE(c_ptr), VALUE, INTENT(in) :: gr !< Output dF/dR array (size = field-7 ndofs)
TYPE(c_ptr), VALUE, INTENT(in) :: gz !< Output dF/dZ array (size = field-7 ndofs)
CHARACTER(KIND=c_char), INTENT(out) :: error_str(OFT_ERROR_SLEN) !< Error string (empty if no error)
TYPE(mugtok_instance), POINTER :: sim_obj
...
CALL sim_obj%sim%get_gradf(gr_tmp,gz_tmp)
END SUBROUTINE mugtok_get_gradf
!---------------------------------------------------------------------------------
!> Get the order-1 (pressure) plotting mesh: node coordinates and triangle connectivity
!---------------------------------------------------------------------------------
SUBROUTINE mugtok_get_pmesh(mugtok_ptr,np,r_loc,nc,lc_loc,error_str) BIND(C,NAME="mugtok_get_pmesh")
TYPE(c_ptr), VALUE, INTENT(in) :: mugtok_ptr !< Pointer to MUG/TokaMaker object
...
!---Tessellate at order 1 -> base mesh (vertices + triangles), matching the order-1 pressure FE
CALL mesh%tessellate(sim_obj%r_pmesh,sim_obj%lc_pmesh,1)
