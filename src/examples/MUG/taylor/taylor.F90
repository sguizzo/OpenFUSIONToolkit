...
TYPE(multigrid_mesh) :: mg_mesh, mg_mesh_p
...
CALL multigrid_construct_surf(mg_mesh_p)
mhd_sim%incomp = .TRUE.
CALL mhd_sim%setup(mg_mesh, order, mg_mesh_p)
...
write(*,*) 'dens: ', m_i*proton_mass*3E24
mhd_sim%nu=nu
mhd_sim%dt=dt
...
!FIX MAGNETIC FIELDS
NULLIFY(mhd_sim%psi_bc) ! Ensure the vector is unallocated (pointer is NULL)
NULLIFY(mhd_sim%by_bc) ! Ensure the vector is unallocated (pointer is NULL)
NULLIFY(mhd_sim%velx_bc) ! Ensure the vector is unallocated (pointer is NULL)
NULLIFY(mhd_sim%vely_bc) ! Ensure the vector is unallocated (pointer is NULL)
NULLIFY(mhd_sim%velz_bc) ! Ensure the vector is unallocated (pointer is NULL)
...
ALLOCATE(mhd_sim%T_bc(oft_blagrange_p%ne))
mhd_sim%psi_bc = .TRUE.
mhd_sim%by_bc = .TRUE.
...
val = SIN(2*pi*pt(1))*COS(2*pi*pt(2))
END SUBROUTINE vx_init

SUBROUTINE vz_init(pt, val)
REAL(r8), INTENT(in) :: pt(3)
REAL(r8), INTENT(out) :: val
val = -1.d0*COS(2*pi*pt(1))*SIN(2*pi*pt(2))
